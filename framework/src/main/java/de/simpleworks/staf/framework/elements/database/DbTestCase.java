...
public class DbTestCase extends TemplateTestCase<DbTeststep, QueuedDbResult> {
	...
	private static QueuedDbResult runQuery(final DbConnectionPool connectionPool, final Statement statement)
			throws SystemException {

		final String connectionId = statement.getConnectionId();

		try (final Connection conn = connectionPool.getConnection(connectionId);) {
			if (conn == null) {
			...
			try (final PreparedStatement selectStatement = conn.prepareStatement(statement.getExpression());
