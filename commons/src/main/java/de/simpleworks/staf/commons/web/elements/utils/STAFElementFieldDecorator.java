...
public class STAFElementFieldDecorator implements FieldDecorator {
	 ...
	 *
	 * @param searchContext The search context for the (custom) webelement. Mostly
	 *                      just a webdriver object. Used to find webelements on a
	 *                      webpage.
	 * @param webDriver     The webDriver which will be used to create the
	 *                      webelement.
	 **/
	public STAFElementFieldDecorator(final SearchContext searchContext, final WebDriver webDriver) {
	 ...
	 * @param field  The field which should be decorated. Should be an FindBy
	 *               annotated (custom) webelement.
	 * @return Value to decorate the field with.
	 **/
	@Override
	public Object decorate(final ClassLoader loader, final Field field) {
		if (STAFElementFieldDecorator.logger.isTraceEnabled()) {
			STAFElementFieldDecorator.logger.trace(String.format("decorate filed: '%s'.", field));
		}

		// If it is a custom annotated webelement, then ensure proper initialisation via
		// the adding of the callback method
		if (STAFElement.class.isAssignableFrom(field.getType()) && field.isAnnotationPresent(FindBy.class)) {
			return getEnhancedObject(field.getType(), getElementHandler(field), field.getAnnotation(FindBy.class));
		}

		// If it is a normal webelement, then use the default FieldDecorator
		// implementation
	 ...
	 **/
	private STAFElementLocator getElementHandler(final Field field) {
	 ...
	 **/
	private ElementLocator getLocator(final Field field) {
	 ...
	 *
	 * @param clzz               The class which should be enhanced with the
	 *                           callback method (e.g. a custom button). If a method
	 *                           in that class is called, the callback method will
	 *                           be triggered.
	 * @param stafElementLocator The class which implements the callback method.
	 * @param locator            The locator which was used to identify the
	 *                           webelement via the FindBy annotation.
	 **/
	private Object getEnhancedObject(final Class<?> clzz, final STAFElementLocator stafElementLocator,
			final FindBy locator) {
		final Enhancer e = new Enhancer();

		e.setSuperclass(clzz);
		e.setCallback(stafElementLocator);

		return e.create(new Class[] { WebDriver.class, By.class },
				new Object[] { webDriver, STAFElementTransformer.transformFindByToBy(locator) });
	}
}
