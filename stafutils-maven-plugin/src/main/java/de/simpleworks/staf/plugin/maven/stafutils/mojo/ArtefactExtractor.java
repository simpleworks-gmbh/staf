	...
	private void writeArtefacts() throws Exception {
		for (final TestcaseReport report : reports) {
			for (final StepReport stepreport : report.getSteps()) {
				...
				UtilsIO.copyFile(attachement,
						Paths.get(targetDirectory, report.getId(),
								stepreport.getDescription().replace(":", "_").replace("\r\n", "_"),
								UtilsDate.getCurrentTimestamp(), attachement.getName()).toFile());
			}
		}
	}
