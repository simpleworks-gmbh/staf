...
public class KafkaClient {
	 ...
	 */
	private static KafkaConsumeResponse getMessages(Consumer consumer,
			KafkaConsumeRequestTimestamp consumedRequestTimestamp, String topic, String key) {
			...
			for (PartitionInfo partition : partitions) {
				...
				case ASCENDING:
					if (!fetchedRecords.addAll(UtilsKafkaConsumer.consumeMessagesAscendingOrder(consumer,
							consumedRequestTimestamp, partition, tp, key, startOffset, totalLengthofPartition))) {
				...
				case DESCENDING:
					if (!fetchedRecords.addAll(UtilsKafkaConsumer.consumeMessagesDescendingOrder(consumer,
							consumedRequestTimestamp, partition, tp, key, totalLengthofPartition, startOffset))) {
						KafkaClient.logger.error(String.format(
