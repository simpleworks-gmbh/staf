...
RUN export MAVEN_LATEST_VERSION=$(curl -s "https://search.maven.org/solrsearch/select?q=g:org.apache.maven+AND+a:apache-maven&core=gav&rows=5&wt=json" | jq -r ".response.docs[] | select(.v | startswith(\"3.\")) | .v" | head -n
 1) && \
